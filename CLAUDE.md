...
- `require_bin` — the "single binary must exist or bail with an apt install hint" dependency check. Only fits the single-binary case (`pdfjam`, `gs`); the Python tools' multi-package apt auto-install differs per tool and stays inline in each script.
