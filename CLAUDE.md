...
- `python_venv_apt_pkgs` — echoes "python3" and/or "python3-venv", whichever apt package the system is missing; both `pdf-contrast-enhancer.sh` and `pdf-signer.sh` feed this into their own `MISSING_APT` array (via `mapfile`) before adding their tool-specific packages.
- `ensure_venv` — creates a persistent venv under `$HOME` if missing and `pip install`s the given packages (every run, so updates are picked up). Used by the contrast-enhancer and signer; callers run `<venv>/bin/python` themselves. Deliberately plain `pip`/`venv`, not `uv`: these venvs are bootstrapped at runtime on whatever machine the tool runs on, not managed as a dev-time project dependency set, so there's no `uv`-on-the-end-user's-machine assumption to make.
...
- `default_output_name` — echoes `<input-basename>_<suffix>.<extension>` (extension default `pdf`), the default filename every tool offers next to its input (`_compressed.pdf`, `_contrast.pdf`, `_slideshow.mp4`, `_signed.pdf`).
...
- **`pdf-compressor.sh`** — pipes Ghostscript's own stderr output through a `while read` loop, regex-matching `Processing pages 1 through N` and `Page N` lines to drive `draw_progress`. Exit status is captured via `PIPESTATUS[0]` since the real command is the left side of a pipe. The gs run lives in a `run_gs` helper. Menu option 4 (target size in MB) runs `compress_to_target`: one pass at the minimum DPI (30; if that doesn't fit, warn and keep it), then a binary search over image DPI up to 300, moving each fitting attempt from a scratch `CANDIDATE` file over the output. It installs its own `EXIT` trap covering both `LOGFILE` and `CANDIDATE`, overriding the one from `init_logfile`.
